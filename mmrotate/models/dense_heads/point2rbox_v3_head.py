...
from mmrotate.models.losses.vis import set_img_id
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            for batch_id in range(len(batch_gt_instances)):
                ...
                if len(mu) >= 1:
                    # Set img_id for organized visualization output
                    if  batch_id < len(batch_img_metas):
                        img_path = batch_img_metas[batch_id].get('img_path', f'batch{batch_id}')
                        set_img_id(img_path)
                    ...
                    if self.loss_pgdm.use_class_specific_watershed:
