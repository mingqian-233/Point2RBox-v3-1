sam_instance_thr = 100000000
sam_batch_size = 128  # SAM 批量推理的 batch size，避免 OOM
sam_enable_predictor_cache = True  # 是否启用predictor缓存（False避免精度下降）
sam_no_batch_inference = False  # 是否禁用批量推理，使用官方单个推理方式（True避免精度下降）
